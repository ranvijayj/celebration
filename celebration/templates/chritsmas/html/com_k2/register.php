...
function validation()
{
	...
	var Mobileno= document.getElementById('url').value;
	re = /^[A-Za-z\s]+$/;
	re_3= /^[A-Za-z\_]+[0-9]*$/;
	var re_2=/[^.[0-9]]*/;
...
<form action="<?php echo JRoute::_('index.php'); ?>"  enctype="multipart/form-data" method="post" id="josForm" name="josForm">
...
<div class="clr"></div>	
<style>
.box1{margin: 20px 0px 0px 174px; height:40px; width:230px; }
</style>
<div class="box1">
<div id="a"  style="font-size:32px; float:left; font-family:'Courier New', Courier, monospace; font-style:italic;" ></div>
<div style="float:left; padding:3px 0px 0px 5px;"><input type="text" id="b"  name="answer" style="margin-top: -10px; font-size:32px; height:32px; width:60px" maxlength="3"/></div>
...
<?php if($this->K2Params->get('recaptchaOnRegistration') && $this->K2Params->get('recaptcha_public_key')): ?>
		...
		<div class="k2AccountPageUpdate" style="border:0px; text-align:justify; margin:0px; padding:0px;">
			<button class="button submit" id="regis" type="submit" style="text-align:justify;  margin-left:200px; font-size:18px">
				<?php echo "Submit"; ?>
			</button>
		</div>
		</div>
