...
function ci()
    {
	 ...
	 window.location="<?php echo Juri::base(true).'/index.php/shopping?view=shopping&what=shopping&cat='.$cat.'&subcat='.$subcat.'&what='.$what.'&where=';?>"+cat;
...
$mailer->setSubject('posted a Query on diwali.getit.in');
$mailer->setBody($body);
$send =& $mailer->Send();
if ( $send !== true ) {
    echo 'Error sending email';
...
font-weight: bold;
color: #BD1919;"><?php echo strtoupper($what); ?></div>
<div class="title_box">
...
<?php $i++; endforeach;
$res=$this->_rescount;
$res=json_encode(json_decode($res),1);
