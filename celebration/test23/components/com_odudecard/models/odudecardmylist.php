...
class odudecardModelodudecardmylist extends JModel
{
  ...
  if($id=='0')
  {
   ...
   $id=$user->username;
	   ...
	   $query = "select id,title,type,file,desp,effect,cat,thumb from #__ecard_media where username='$id' order by ddate desc";
	   $db->setQuery( $query );
