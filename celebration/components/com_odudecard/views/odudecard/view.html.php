...
$rows = $model->getGreeting($cardPerPage);
	...
	$f=0;
...

		{
			...
			$tab.='<td valign=top width=114 align=center ><a href='.JRoute::_('index.php?option=com_odudecard&id='.$id.'&controller=odudecardshow&Itemid='.$mymenuitem.'&cate='.$cate).' ><img src="'.JURI::base().'images/ecard/'.$thumb.'" alt="'.$title.'" border=1 id=card2><br /></a><a href='.JRoute::_('index.php?option=com_odudecard&id='.$id.'&controller=odudecardshow&Itemid='.$mymenuitem.'&cate='.$cate).' style="text-decoration:none;color:#000;" >'.$title.'</a></td>';
