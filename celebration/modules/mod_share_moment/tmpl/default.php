...
$fileName=$_FILES['file']['name'];
	$ext = substr($fileName,strrpos($fileName,'.')+1);
	 if ($fileName && !(($ext == "jpg" || $ext == "gif" || $ext == "png") && ($_FILES["file"]["type"] == "image/jpeg" || $_FILES["file"]["type"] == "image/gif" || $_FILES["file"]["type"] == "image/png") && ($_FILES["file"]["size"] < 1120000))){//echo 'dsbndsbj';die;
	
			$link = 'index.php/share-moment';
			$msg = 'Error uploading image .Please upload jpg,png,gif format with size smaller than 1mb. Please try again.';
...
if(move_uploaded_file($_FILES['file']['tmp_name'], $target_path.DS.'src'.DS.$fileName))

 {
    ...
    echo "<div style='font-size: 15px;font-weight: bold;color: red;'>Your moment has been shared successfully</div>";
...
<div class="share_input_box_one"><input type="submit" name="save" value="Submit" class="share_submit_btn" style="cursor:pointer;">
