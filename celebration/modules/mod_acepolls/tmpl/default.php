...
<?php for ($i = 0, $n = count($options); $i < $n; $i ++) { ?>
	...
	<label for="mod_voteid<?php echo $options[$i]->id;?>" class="<?php echo $tabclass_arr[$tabcnt].$params->get('moduleclass_sfx'); ?>" style="display:block; padding:2px;">
		...
		<div style="float:left; background:none; width:20px; margin:3px 0px 0px 0px"><input type="radio" name="voteid" id="mod_voteid<?php echo $options[$i]->id;?>" value="<?php echo $options[$i]->id;?>" alt="<?php echo $options[$i]->id;?>" <?php echo $disabled; ?> /></div>

		<div style="float:left; background:none; margin:0px 0px 0px 5px; font-weight:bold;"><?php echo $options[$i]->text; ?></div>
	...
	<div class="voteBtn" style="margin:22px 0px 0px 146px;" id="poll_buttons_<?php echo $poll->id;?>">	

	<input type="image" style="width:auto;" src="images/vote.png" width="60" height="27" id="submit_vote_<?php echo $poll->id; ?>" name="task_button" <?php echo $disabled; ?> />

	<input type="hidden" name="task_button">
...
//If user has voted 
		...
		foreach ($results as $row) :
			...
			<div>
				...
				<div style="height:10px; width:122px; padding:1px; border:1px solid #<?php echo $params->get('poll_bars_border_color'); ?>;">
...
<?php /*?><?php if (($params->get('show_view_details')) || ($params->get('rel_article_window'))) { ?>
